...
TARGETDIR=data/with_rt_intronic
BALLGOWN=$HOME/thesis_projects/ensembl_ballgown/data/ballgown/cell_de_results_transcripts.tsv

cat $BEDDIR/B/transcripts_rt_overlap_summ+gffcmp.bed | grep -P "\si\s" | awk '{print $9}' | sort | uniq > $TARGETDIR/B_ensembl_with_intronic.txt
cat $BEDDIR/T/transcripts_rt_overlap_summ+gffcmp.bed | grep -P "\si\s" | awk '{print $9}' | sort | uniq > $TARGETDIR/T_ensembl_with_intronic.txt
...
$BALLGOWN \
imgs/volcano_ensembl_with_rt_intronic_cell_de.svg ""
